...
// Variável global para controlar a interrupção do botão
volatile bool botao_pressionado = false;

// Função de interrupção para detectar o botão pressionado
void interrupcao_botao(uint gpio, uint32_t events) {
    if (events & GPIO_IRQ_EDGE_FALL) { // Detecta queda de nível (botão pressionado)
...
int main() {
  ...
  while (true) {
      if (botao_pressionado) {
          ...
          botao_pressionado = false; // Resetando a flag
      } else {
