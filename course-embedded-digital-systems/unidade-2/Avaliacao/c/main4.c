...
ISR(INT0_vect)
{
  if (PIND & (1 << PD2)) {  // Tenta evitar deboucing na descida do botão
    somar = 1;
  }
}
...
ISR(INT1_vect)
{
  if (PIND & (1 << PD3)) {  // Tenta evitar deboucing na descida do botão
    decrementar = 1;
  }
}
...
int conversor(int bin) // int para uint8_t
{
  bin ^= (bin >> 1);
  return bin;
}
...
int main(void)
{
  ...
  DDRC |= (0X3);                  // set ouput PC0, PC1 11 // OB = xxxx_xx11 -> DEC = 3 -> 0X = 3
  ...
  EICRA = ((1 << ISC01) | (1 << ISC00)|(1 << ISC11) | (1 << ISC10));                      // mudança na borda de subida = 11
  EIMSK |= ((1 << INT0) | (1 << INT1)); // INT0 e INT1  activate

  TCCR0A = 0b00000000; // TC0 (normal operating)
  TCCR0B = 0b00000010; // TC0 with prescaler 8. Overflow every 128us = 256/(16Mhz/8) T = 2^n / F  | F = 16M / 8 | 2^n = 256 [255, 0]
